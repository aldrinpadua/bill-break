// Bill Break — email someone when they're invited to, or added to, a group.
...
//   FROM_EMAIL       — a verified sender, e.g. "Bill Break <hello@aldrinpadua.com>"
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (type === "invite") {
      subject = `${who} invited you to "${groupName}" on Bill Break`;
      body = `${greeting}\n\n${who} is splitting expenses for "${groupName}" on Bill Break and added you.\n\nJoin here — sign in with this email (${email}) and you'll automatically be part of "${groupName}":\n${link}\n\nBill Break makes it easy to split trips and bills with friends.\n\n— Bill Break`;
    } else {
      subject = `${who} added you to "${groupName}" on Bill Break`;
      body = `${greeting}\n\n${who} added you to "${groupName}" on Bill Break. Open it here to see the shared expenses and what's owed:\n${link}\n\n— Bill Break`;
    }
