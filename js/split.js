...
export function computeOwed(expense) {
  ...
  if (s.type === "items") {
    // Each item is split equally among its own participants. Tax and gratuity
    // are then shared out in proportion to what each person actually ordered
    // (their item subtotal) — so someone who ordered more pays more tax/tip.
    const subtotal = new Map(); // per-person items subtotal (insertion-ordered)
    for (const item of s.items || []) {
      const parts = item.participants || [];
      const shares = splitEvenly(item.amountMinor, parts.length);
      parts.forEach((id, i) => {
        add(id, shares[i]);
        subtotal.set(id, (subtotal.get(id) || 0) + shares[i]);
      });
    }
    const ids = [...subtotal.keys()];
    const weights = ids.map((id) => subtotal.get(id));
    for (const extra of [s.taxMinor || 0, s.tipMinor || 0]) {
      if (!extra) continue;
      const shares = splitByWeights(extra, weights); // proportional, cent-exact
      ids.forEach((id, i) => add(id, shares[i]));
    }
    // Back-compat: older expenses used a flat "shared" pool split evenly.
    if (s.sharedMinor) {
      const pool = (s.sharedParticipants && s.sharedParticipants.length) ? s.sharedParticipants : ids;
      const shares = splitEvenly(s.sharedMinor, pool.length);
...
export function validateExpense(expense) {
  ...
  if (expense.split?.type === "items") {
    ...
    const itemsTotal = (s.items || []).reduce((a, it) => a + it.amountMinor, 0) + (s.taxMinor || 0) + (s.tipMinor || 0) + (s.sharedMinor || 0);
    if (itemsTotal !== expense.amountMinor) errors.push(`Subtotal + tax + tip (${itemsTotal}) ≠ total (${expense.amountMinor}).`);
