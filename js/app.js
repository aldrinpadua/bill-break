...
function renderFriendDetail(friendId) {
  ...
  if (rb) rb.onclick = async () => {
    ...
    const msg = `Hi ${m.name},\n\nJust a friendly reminder about what's outstanding between us on Bill Break:\n\n${lines.join("\n")}\n\nThanks!\n— ${store.state.you.name}`;
...
function renderMembers(body, l) {
  const membersHtml = l.memberIds.map((id) => {
    ...
    return `<div class="bal-row">
      ...
      <div class="grow" data-profile="${id}" style="cursor:pointer"><b>${esc(m?.name)}</b>${id === "you" ? " (you)" : ""}${handle} ${meta}${pending ? ` <span class="tag" style="color:var(--amber)">invited · not signed up yet</span>` : ""}</div>
