  ...
  if (!ledgers.length) {
    ...
    list.innerHTML = ledgers.map((l) => {
      ...
      return `<div class="exp-row" data-ledger="${l.id}" style="cursor:pointer">
        ...
        <div class="exp-main"><div class="exp-desc">${esc(l.name)}${parent ? ` <span class="tag">in ${esc(parent.name)}</span>` : ""}</div>
...
function renderMembers(body, l) {
  const membersHtml = l.memberIds.map((id) => {
    ...
    return `<div class="bal-row">
      ...
      <div class="grow"><b>${esc(m?.name)}</b>${id === "you" ? " (you)" : ""} <span class="exp-meta">${m?.email ? esc(m.email) : "no email"}</span>${pending ? ` <span class="tag" style="color:var(--amber)">invited · not signed up yet</span>` : ""}</div>
  ...
  if (CLOUD) {
    addHtml = `
      <h3 style="margin:20px 0 10px">Add people by email</h3>
      <div class="card">
        <div class="hint">Add someone by their email. If they already use Bill Break they join instantly; if not, you'll get an invite link to send, and they join automatically once they sign up with that email.</div>
        <div class="row" style="margin-top:12px">
          <input id="memEmail" type="email" inputmode="email" placeholder="name@example.com">
          <button class="btn" id="memAdd" style="flex:none">Add / Invite</button>
        </div>
  ...
  if (CLOUD) {
    $("#memAdd").onclick = async () => {
      const email = $("#memEmail").value.trim(); if (!email) return toast("Enter an email.");
      ...
      btn.disabled = false;
      if (res.status === "added") { toast(`Added ${res.name}.`); render(); }
...
function openInvite(l, res) {
  const msg = `Join me on Bill Break for "${l.name}"!\n\nSign in with ${res.email} here:\n${res.link}\n\nYou'll automatically join "${l.name}" once you sign in with that email.`;
  modal("Invite link", `
    <p class="hint" style="margin-top:0"><b>${esc(res.email)}</b> isn't on Bill Break yet, so I added them as <b>pending</b>. Send them the message below — they'll join "${esc(l.name)}" automatically when they sign up with that email.</p>
    <div class="mail-preview" id="invMsg">${esc(msg)}</div>
  `, `<button class="btn ghost" data-close>Close</button><button class="btn" id="copyInv">Copy message</button>`);
  $("#copyInv").onclick = async () => { try { await navigator.clipboard.writeText(msg); toast("Invite copied — paste it to your friend."); } catch { toast("Copy failed — select the text and copy it."); } };
}
...
function openLedgerModal(kind) {
  ...
  const nameLabel = kind === "individual" ? (CLOUD ? "Friend's email" : "Friend's name") : "Name";
  const namePlaceholder = kind === "trip" ? "Tokyo 2026" : kind === "group" ? "College Friends" : (CLOUD ? "friend@example.com" : "Alex");
  modal(`New ${kindLabel[kind].toLowerCase()}`, `
    <label>${nameLabel}</label>
    <input id="mName" ${cloudFriend ? 'type="email" inputmode="email"' : ""} placeholder="${namePlaceholder}">
    ...
    ${cloudFriend
      ? `<div class="hint" style="margin-top:14px">If they already use Bill Break they're added instantly; if not, you'll get an invite link to send them.</div>`
      : (CLOUD
  ...
  $("#mCreate").onclick = async () => {
    ...
    if (!input) return toast(cloudFriend ? "Enter their email." : "Enter a name.");

    // Cloud 1:1 friend: create the ledger, then add/invite them by email.
    if (cloudFriend) {
      if (!input.includes("@")) return toast("Enter a valid email.");
      const btn = $("#mCreate"); btn.disabled = true;
      const l = store.addLedger({ kind, name: input.split("@")[0], baseCurrency: $("#mCur").value, memberIds: [] });
      const res = await store.addMemberByEmail(l.id, input);
      if (res.status === "added") { store.updateLedger(l.id, { name: res.name }); toast(`Added ${res.name}.`); }
      else if (res.status === "exists") { toast(res.message); }
...
function mobileBar() { return `<div class="mobile-bar"><button class="hamburger" id="hamburger">☰ Menu</button><b>Bill Break</b></div>`; }
...
document.querySelectorAll(".nav-item[data-view='dashboard']").forEach((b) => b.onclick = () => { view = { type: "dashboard" }; setSidebar(false); render(); });
