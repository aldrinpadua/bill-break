...
function openExpenseModal(ledgerId, editId) {
  ...
  const st = {
    ...
    items: null,   // lazily built {items:[{name,amount,parts:Set}], shared, sharedParts:Set}
  };
  ...
  if (existing?.split?.type === "items") {
    st.items = {
      ...
      shared: existing.split.sharedMinor ? toMajor(existing.split.sharedMinor, existing.currency) : "",
      sharedParts: new Set(existing.split.sharedParticipants || participantsOf(existing)),
    };
  }
  ...
  function currentSplit() {
    ...
    if (st.method === "items") {
      ...
      const src = st.items || { items: [], shared: "", sharedParts: new Set() };
      ...
      return { type: "items", items, sharedMinor: toMinor(src.shared || 0, cur), sharedParticipants: [...src.sharedParts] };
    }
  ...
  function updateStatus() {
    ...
    if (st.method === "percent") {
      ...
      const s = currentSplit(); const itot = (s.items || []).reduce((a, i) => a + i.amountMinor, 0) + (s.sharedMinor || 0);
      el.innerHTML = `Items + shared = <b>${formatMoney(itot, st.currency)}</b> of <b>${formatMoney(totalMinor, st.currency)}</b>. ${itot === totalMinor ? "✅" : "⚠️ must match total"}`;
    } else {
  ...
  $("#eSave").onclick = () => {
    const amountMinor = toMinor(amtEl.value || 0, st.currency);
    if (amountMinor <= 0) return toast("Enter an amount.");
...
function renderItemsEditor(area, members, st, onChange) {
  if (!st.items) st.items = { items: [{ name: "", amount: "", parts: new Set(st.participants) }], shared: "", sharedParts: new Set(st.participants) };
  const S = st.items;
  const draw = () => {
    area.innerHTML = `
      <div class="hint">Assign each item to who shared it. Tax/tip/fees go in the shared pool and split across everyone below.</div>
      ${S.items.map((it, i) => `
      ...
      <label>Shared pool — tax, tip, service (split across everyone on any item)</label>
      <input id="iShared" inputmode="decimal" placeholder="0.00" value="${esc(S.shared)}">
    `;
    area.querySelector("#addItem").onclick = () => { S.items.push({ name: "", amount: "", parts: new Set(st.participants) }); draw(); };
    area.querySelectorAll("[data-iname]").forEach((el) => el.oninput = () => { S.items[+el.dataset.iname].name = el.value; });
    area.querySelectorAll("[data-iamt]").forEach((el) => el.oninput = () => { S.items[+el.dataset.iamt].amount = el.value; refreshShared(); onChange && onChange(); });
    area.querySelector("#iShared").oninput = (e) => { S.shared = e.target.value; onChange && onChange(); };
    area.querySelectorAll("[data-ip]").forEach((c) => c.onclick = () => { const [i, id] = c.dataset.ip.split(":"); const p = S.items[+i].parts; p.has(id) ? p.delete(id) : p.add(id); refreshShared(); draw(); onChange && onChange(); });
    area.querySelectorAll("[data-irm]").forEach((b) => b.onclick = () => { S.items.splice(+b.dataset.irm, 1); if (!S.items.length) S.items.push({ name: "", amount: "", parts: new Set(st.participants) }); refreshShared(); draw(); onChange && onChange(); });
  };
  // shared pool participants = union of everyone appearing on any item
  const refreshShared = () => { S.sharedParts = new Set(S.items.flatMap((it) => [...it.parts])); };
  draw();
}
