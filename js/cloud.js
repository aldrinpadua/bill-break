...
class CloudStore {
  ...
  async setUsername(name) {
    try {
      ...
      if (data && data.ok) { this.state.you.username = data.username; this._notify(); return { ok: true }; }
  ...
  async addMemberByEmail(ledgerId, rawInput) {
    ...
    if (user) {
      ...
      if (!person) { person = { id: user.id, name: label, email: email || "", username: user.username || null, userId: user.id }; this.state.people.push(person); }
      else { person.userId = user.id; if (email) person.email = email; if (user.username) person.username = user.username; }
