...
class CloudStore {
  ...
  async hydrate() {
    ...
    you.email = you.email || myEmail;

    const { data: ledgers = [] } = await this.sb.from("ledgers").select("*");
    ...
    // people = every non-me member, deduped by ref
    const peopleMap = new Map();
    for (const m of members) {
      const ref = mapRef(m.member_ref, myId, "you");
      if (ref === "you") continue;
      if (!peopleMap.has(ref)) peopleMap.set(ref, { id: ref, name: m.name, email: m.email || "" });
    }
    ...
    this.state.ledgers = ledgers.map((l) => ({
      ...
      memberIds: members.filter((m) => m.ledger_id === l.id).map((m) => mapRef(m.member_ref, myId, "you")),
      reminder: l.reminder || { enabled: false, frequency: "weekly", lastSentAt: null, message: "" },
      createdAt: new Date(l.created_at).getTime(),
    }));
    this.state.expenses = expenses.map((e) => ({ id: e.id, ledgerId: e.ledger_id, createdAt: new Date(e.created_at).getTime(), ...translateExpenseData(e.data, myId, "you") }));
    this._notify();
  }
  ...
  _memberRow(ledgerId, ref) {
    const dbRef = mapRef(ref, "you", myId);
    if (ref === "you") return { ledger_id: ledgerId, member_ref: dbRef, name: this.state.you.name, email: this.state.you.email || myEmail, user_id: myId };
    const p = this.state.people.find((x) => x.id === ref);
    return { ledger_id: ledgerId, member_ref: dbRef, name: p?.name || "Friend", email: p?.email || null, user_id: null };
  }
  ...
  updatePerson(id, patch) {
    if (id === "you") {
      ...
      this._try("member self update", () => this.sb.from("ledger_members").update({ name: this.state.you.name, email: this.state.you.email }).eq("member_ref", myId));
      return;
    }
  ...
  updateLedger(id, patch) {
    ...
    this._try("ledger update", async () => {
      ...
      if (patch.memberIds) {
        ...
        for (const ref of removed) await this.sb.from("ledger_members").delete().eq("ledger_id", id).eq("member_ref", mapRef(ref, "you", myId));
      }
    });
  }
  removeLedger(id) {
  ...
  addExpense(exp) {
    ...
    this._try("expense insert", () => this.sb.from("expenses").insert({ id, ledger_id: ledgerId, data: translateExpenseData(data, "you", myId), created_by: myId }));
    return e;
  }
  updateExpense(id, patch) {
    ...
    this._try("expense update", () => this.sb.from("expenses").update({ data: translateExpenseData(data, "you", myId) }).eq("id", id));
  }
  removeExpense(id) {
