...
class CloudStore {
  ...
  async addMemberByEmail(ledgerId, rawEmail) {
    ...
    if (user && user.id !== myId) {
      ...
      this._try("notify added", () => this.sb.functions.invoke("notify-member", { body: { type: "added", email, name: person.name, groupName: l.name, inviterName: this.state.you.name, link: appUrl() } }));
    ...
    this._try("notify invite", () => this.sb.functions.invoke("notify-member", { body: { type: "invite", email, name: person.name, groupName: l.name, inviterName: this.state.you.name, link: appUrl() } }));
    return { status: "invited", email, name: person.name, link: appUrl() };
  }
