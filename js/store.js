...
//
// Phase 1 (now): everything is kept in memory and persisted to the browser's
// localStorage, so the app works instantly on GitHub Pages with zero backend.
//
// Phase 2 (later): swap `Backend.local` for `Backend.supabase` (see
// js/supabase-adapter.js and README). The rest of the app talks only to the
// `store` API below, so no UI code changes when you switch.
...
class LocalStore {
  ...
  importJSON(text) {
    ...
    if (!data || !data.version) throw new Error("Not a Bill Break backup file.");
    this.state = data;
    this._persist();
  }
